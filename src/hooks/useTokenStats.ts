...
export type StatsProvider = "claude" | "codex" | "opencode" | "kimi" | "glm" | "hermes";
...
export function useTokenStats(provider: StatsProvider = "claude") {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const command = provider === "codex" ? "get_codex_stats" : provider === "opencode" ? "get_opencode_stats" : provider === "kimi" ? "get_kimi_stats" : provider === "glm" ? "get_glm_stats" : provider === "hermes" ? "get_hermes_stats" : "get_all_stats";
